#!/usr/bin/env python3
"""
Experiment Runner for OpenHands + GitHub Actions

This script executes individual experiment steps as defined in experiments.yaml.
OpenHands will monitor CI and iterate on any failures using AGENTS.md guidance.

Usage:
    python runner.py --spec experiments/experiments.yaml --output artifacts/
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Experiment Runner for OpenHands + GitHub Actions")
    parser.add_argument(
        "--spec",
    ...
    try:
        runner = ExperimentRunner(args.spec, args.output)

        if args.step:
            success, result = runner.run_step(args.step)
            # For OpenHands CI monitoring, we want to fail the CI step if sanity checks fail
            # This will trigger OpenHands to iterate and fix the issue
            if not (success and result.get('sanity_passed', False)):
                logger.error(f"Step {args.step} failed - OpenHands will iterate to fix this")
                sys.exit(1)
        else:
            ...
            if not success:
                logger.error("Some steps failed - OpenHands will iterate to fix issues")
                sys.exit(1)
