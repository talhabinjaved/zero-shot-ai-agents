#!/usr/bin/env python3
"""
Experiment Executor for Cosine + GitHub Actions

This script executes individual experiment steps as defined in experiments.yaml.
Cosine will monitor CI steps and auto-iterate until sanity checks pass.

Usage:
    python executor.py --spec experiments/experiments.yaml --output artifacts/
"""
...
class ExperimentExecutor:
    ...
    def _execute_command(self, cmd: str, timeout_minutes: int = 60) -> Tuple[bool, str, int]:
        ...
        try:
            ...
            log_file = self.output_dir / "logs" / f"step_{cmd.split()[0]}_{int(time.time())}.log"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Experiment Executor for Cosine + GitHub Actions")
    parser.add_argument(
        "--spec",
    ...
    try:
        executor = ExperimentExecutor(args.spec, args.output)

        if args.step:
            success, result = executor.run_step(args.step)
            # For Cosine CI monitoring, we want to fail the CI step if sanity checks fail
            # This will trigger Cosine to iterate and fix the issue
            if not (success and result.get('sanity_passed', False)):
                logger.error(f"Step {args.step} failed - Cosine will iterate to fix this")
                sys.exit(1)
        else:
            results = executor.run_all_steps()
            success = results.get('all_success', False)
            logger.info(f"Experiment execution completed. Success: {success}")
            if not success:
                logger.error("Some steps failed - Cosine will iterate to fix issues")
                sys.exit(1)
